...
function initializeApp() {
    ...
    cepInput.addEventListener('blur', validateAndSearchCEP);
...
function formatCEP(e) {
    ...
    // Limpar informações de endereço se o CEP foi modificado
    if (addressInfo.style.display === 'block') {
        addressInfo.style.display = 'none';
        currentAddress = null;
    }
}

// Função para validar e buscar CEP
async function validateAndSearchCEP() {
    ...
    if (cep.length === 8) {
        try {
            ...
            if (address && !address.erro) {
                ...
                hideLoadingCEP();
                showNotification('CEP não encontrado. Verifique e tente novamente.', 'error');
                addressInfo.style.display = 'none';
            }
        } catch (error) {
            hideLoadingCEP();
...
// Função para mostrar loading do CEP
function showLoadingCEP() {
    const icon = cepInput.parentElement.querySelector('.search-icon');
    icon.className = 'fas fa-spinner fa-spin search-icon';
}

// Função para esconder loading do CEP
function hideLoadingCEP() {
    const icon = cepInput.parentElement.querySelector('.search-icon');
    icon.className = 'fas fa-map-marker-alt search-icon';
}

// Função para mostrar informações do endereço
function showAddressInfo(address) {
    const fullAddress = `${address.localidade}, ${address.uf}`;
    addressText.textContent = fullAddress;
    addressInfo.style.display = 'block';
    
    // Animação suave
    addressInfo.style.opacity = '0';
    addressInfo.style.transform = 'translateY(-10px)';
    
    setTimeout(() => {
        addressInfo.style.transition = 'all 0.3s ease';
        addressInfo.style.opacity = '1';
        addressInfo.style.transform = 'translateY(0)';
    }, 100);
}
    ...
    navigator.geolocation.getCurrentPosition(
        function(position) {
            ...
            reverseGeocode(userLocation.lat, userLocation.lng)
                .then(address => {
                    currentAddress = {
                        localidade: address.split(',')[0].trim(),
                        uf: address.split(',')[1].trim()
                    };
                    showAddressInfo(currentAddress);
                    hideLoading();
...
function handleSearch() {
    ...
    if (!currentAddress && cep.length !== 8) {
    ...
    if (!currentAddress) {
        showNotification('Aguarde a validação do CEP', 'warning');
        return;
    }
    
    // Mostrar loading com texto personalizado
    const locationText = `${currentAddress.localidade}, ${currentAddress.uf}`;
    showLoading(`Buscando peneiras próximas a ${locationText}...`);
...
function searchPeneiras(address) {
    try {
        // Simular geocodificação da localização
        const userCoords = geocodeLocation(`${address.localidade}, ${address.uf}`);
...
function showLoading(message = 'Buscando peneiras...') {
